...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (3006575,35918319)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (3006575,35918319)
  and c.invalid_reason is null
...
from 
(
  select m.person_id,m.measurement_id,m.measurement_concept_id,m.visit_occurrence_id,m.value_as_number,m.range_high,m.range_low,m.value_as_concept_id,m.measurement_date as start_date, DATEADD(day,1,m.measurement_date) as end_date , row_number() over (PARTITION BY m.person_id ORDER BY m.measurement_date, m.measurement_id) as ordinal
  FROM @cdm_database_schema.MEASUREMENT m
JOIN #Codesets cs on (m.measurement_concept_id = cs.concept_id and cs.codeset_id = 1)
) C

WHERE C.value_as_concept_id in (35919824,35919827,35919263,35919747,35919151,35919820,35919762,35919199,35919922,35919359,35919520,35919183,35919664,35919321,35919223,35919795,35919415,35919132,35919480,35919220,35919375,35919269,35919431,35919430,35939367)
AND C.ordinal = 1
...
FROM #qualified_events E
JOIN @cdm_database_schema.PERSON P ON P.PERSON_ID = E.PERSON_ID
WHERE P.gender_concept_id in (45438358,8507)
GROUP BY e.person_id, e.event_id
-- End Demographic Criteria
